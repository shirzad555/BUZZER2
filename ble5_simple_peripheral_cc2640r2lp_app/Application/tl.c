...
static ICall_SyncHandle *appSem = NULL;
...
//  Semaphore_post(*appSem);
  Event_post(*appSem, TX_DONE_ISR_EVENT_FLAG);
}
...
//  Semaphore_post(*appSem);
  Event_post(*appSem, TRANSPORT_RX_ISR_EVENT_FLAG);
}
...
//  Semaphore_post(*appSem);
  Event_post(*appSem, MRDY_ISR_EVENT_FLAG);
}
...
#ifdef POWER_SAVING
    //perform handshaking
//    NPITL_handleMrdyEvent(); // Shirzad removed this
#endif //POWER_SAVING
  }
...
//      Semaphore_post(*appSem);
      Event_post(*appSem, transport_rx_event);
    }
  }
...
//! \return     void
// -----------------------------------------------------------------------------
void TLinit(ICall_SyncHandle *pAppSem, TLCBs_t *appCallbacks, uint16_t TX_DONE_EVENT, // void TLinit(ICall_Semaphore *pAppSem, TLCBs_t *appCallbacks, uint16_t TX_DONE_EVENT,
